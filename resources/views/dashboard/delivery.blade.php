    ...
    <div class="relative w-full flex flex-col h-screen overflow-y-hidden">
        ...
        <div class="w-full h-screen overflow-x-hidden border-t flex flex-col">
            <main class="w-full flex-grow p-6">
                ...
                <form action="{{ route('delivery_fees.store') }}" method="POST" class="mb-6 bg-white p-6 rounded-lg shadow-md border border-gray-200">
                    @csrf
                    <h2 class="text-xl font-semibold text-gray-800 mb-4">Ajouter une frais</h2>
                    <div class="flex flex-wrap space-y-4 md:space-y-0 md:space-x-4">
                        <div class="w-full md:w-2/3">
                            ...
                            <input
                                ...
                                name="city"
                                id="city"
                                placeholder="Entrez le nom de la city"
    ...
    e.target.value = e.target.value.replace(/(?:^|\s)\S/g, function(a) { return a.toUpperCase(); });
                ...
                <div class="w-full mt-6">
                    <p class="text-xl pb-3 flex items-center text-gray-700">
                        <i class="fas fa-list mr-3 text-green-600"></i> Frais Des Livraisions
                    </p>
                    <div class="bg-white overflow-auto shadow-md rounded-lg">
                        <table class="min-w-full bg-white border border-gray-200 rounded-lg">
                            <thead class="bg-green-700 text-white">
                                <tr>
                                    <th class="text-left py-4 px-6 uppercase font-semibold text-sm border-b border-gray-200">
                                        Ville
                                    </th>
                                    <th class="text-left py-4 px-6 uppercase font-semibold text-sm border-b border-gray-200">
                                        Frais (MAD)
                            ...
                            <tbody class="text-gray-700">
                                ...
                                <tr class="hover:bg-green-50">
                                    ...
                                    <td class="text-left py-4 px-6 border-b border-gray-200">
                                        <span class="bg-green-200 text-green-700 text-sm px-2 py-1 rounded">
                                            {{ number_format($fee->fee, 2) }} MAD
                                        </span>
                                    </td>
                                    <td class="text-left py-4 px-6 border-b border-gray-200 flex gap-2">
                                        <a href="{{ route('delivery_fees.edit', $fee) }}" class="px-3 py-2 bg-yellow-500 text-white text-sm font-medium rounded hover:bg-yellow-600">
                                            Modifier
                                        </a>
                                        <form action="{{ route('delivery_fees.destroy', $fee) }}" method="POST" style="display:inline-block;">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="px-3 py-2 bg-red-600 text-white text-sm font-medium rounded hover:bg-red-700" onclick="return confirm('Confirmer la suppression ?')">
                                                Supprimer
                                            </button>
                                        </form>
                                    </td>
